...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `ask_user_question`:
...
- **Technical preferences**: Read `.pi/game-studio/technical-preferences.md` — extract performance budgets and naming conventions.

---
name: brainstorm
description: "🎮 [Studio] Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   2. Else → default to `lean`
   3. Review mode pipeline follows the unified solo/lean/full pattern from /setup and /start skills.
...
2. **Check for existing concept work**:
   ...
   - **Engram check**: Use silent detection via `which engram` command to check for memory capabilities. If available, query saved brainstorm checkpoints using the topic keys `brainstorm/phase-1`, `brainstorm/phase-2`, etc. and offer to resume from the latest checkpoint.
...
- What 3 games have you spent the most time with? What kept you coming back?
  ...
  Do NOT put this in an ask_user_question with preset options.)_
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? _(Use `ask_user_question` for this — constrained choice.)_
...
**After identifying biggest technical risks, spawn `technical-director` via subagent using gate TD-FEASIBILITY (shared director-gate pattern) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via subagent using gate PR-SCOPE (shared director-gate pattern).**
...
4. **Generate the game concept document** using the template at
   `prompts/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `prompts/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.

...
5. **Production** — Feature development (Epic/Feature/subagent tracking active)
...
- [ ] Technical preferences configured (`.pi/game-studio/technical-preferences.md` populated)
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via subagent using the parallel gate protocol from `.pi/game-studio/director-gates.md`. Issue all four subagent calls simultaneously — do not wait for one before starting the next.
...
- **No accessibility requirements doc?** → Use `ask_user_question` to offer to create it now:
  ...
  - If "Create it now": use a second `ask_user_question` to ask for the tier:
  ...
  - Then write `design/accessibility-requirements.md` using the template at `.pi/game-studio/templates/accessibility-requirements.md`, filling in the chosen tier. Confirm: "May I write `design/accessibility-requirements.md`?"

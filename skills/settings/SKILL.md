...
Phase 5 follow the pattern in `prompts/automation-modes.md`. **Note**:
...
If `scripts/yaml-helper.sh` is missing, this is not a Pi Game Studio project.
...
```bash
grep -B14 '^> ### RESERVED' prompts/effects-map.md | grep -E '^## '
```
...
- For **which value to choose** for a given game type or situation — and when to
  suggest changing one — see `prompts/settings-guidance.md` (advisory presets

---
name: skill-test
description: "🎮 [Studio] Validate skill files for structural compliance and behavioral correctness. Three modes: static (linter), spec (behavioral), audit (coverage report)."
agent: qa-lead
model: inherit
...
- `category [name]` → run category-specific rubric from `skills/test-rubrics/quality-rubric.md`
...
Look up the spec path from `skills/test-catalog.yaml` — use the
...
Date: [date]
Spec: skills/test-results/specs/[category]/[name].md
...
"May I write these results to `skills/test-results/skill-test-spec-[name]-[date].md`
and update `skills/test-catalog.yaml`?"

If yes:

- Write results file to `skills/test-results/skill-test-spec-[name]-[date].md`
- Update the skill's entry in `skills/test-catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `skills/[name]/SKILL.md`.
Look up `category:` field in `skills/test-catalog.yaml`.
...
Read `skills/test-rubrics/quality-rubric.md`.
...
"May I update `skills/test-catalog.yaml` to record this category check
...
Read `skills/test-catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).
...
Also read the `agents:` section from `skills/test-catalog.yaml` to get the
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `skills/test-results/specs/[category]/[name].md`)
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `skills/test-results/specs/agents/[name].md`)
...
- After `spec [name]` PASS: "Update `skills/test-catalog.yaml` to record this
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `prompts/skill-test-spec.md` to create new specs."

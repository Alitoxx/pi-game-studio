...
- Naming conventions — `naming.*` from `project.yaml`; for any key absent or empty (including when `project.yaml` has no `naming` block), from `production/technical-preferences.md`
...
Read `prompts/vertical-slice-report.md` to get the report structure.
...
- `full` → spawn `creative-director` via `Agent` using gate **CD-PLAYTEST**
  (`.pi/game-studio/director-gates/cd-playtest.md`).
